import { Check, Plus, Sparkles, ShieldCheck, Tag, DollarSign } from 'lucide-react';
...
export function ProductCard({ product, item, line, onToggle, onChange }: Props) {
  ...
  return (
    <div
      className={`rounded-2xl border transition-all ${
        included
          ? 'border-brand-200 bg-white shadow-glow'
    ...
    >
        ...
        <button
          ...
          className={`shrink-0 mt-0.5 h-6 w-6 rounded-lg border transition flex items-center justify-center ${
            included
              ...
              : 'bg-white border-ink-300 hover:border-brand-400 text-transparent'
          }`}
      ...
      {included && (
        <div className="border-t border-ink-100 p-4 pt-3 grid grid-cols-12 gap-3 bg-ink-50/30 rounded-b-2xl">
          {product.isAdSpend && (
            <div className="col-span-12 sm:col-span-6">
              ...
              {product.minAdSpend && (
                <div className="stat-sub">
                  Minimum {fmt(product.minAdSpend)}/mo
                </div>
              )}
            </div>
          )}
          ...
          <div className="col-span-12 mt-1 pt-3 border-t border-dashed border-ink-200 grid grid-cols-3 gap-3 text-center">
            <div>
              ...
              <div className="text-sm font-bold text-ink-900 num">
            ...
            <div>
              ...
              <div className="text-sm font-bold text-brand-600 num">
                {fmt(line.monthlyCommission)}
              </div>
            </div>
          </div>
        </div>
